...
@WebServlet("/WonAuctionsListRIA")
public class WonAuctionsListRIA extends HttpServlet {
	@Serial
	private static final long serialVersionUID = 1L;
	private Connection connection = null;

	public WonAuctionsListRIA() {
		super();
	}
	...
	private void setupPage(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (checkConnection(connection)) {
			...
			if (wonAuctions.size() != 0) {
				json = gson.toJson(wonAuctions);
			} else {
